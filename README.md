...
| 交给 AI 助手（自动） | 同上，但复制粘贴那两步由工具代你做：提示词从 stdin 送进去，正文从 stdout 接回来。录完点一次分析就出纪要。 | `assistants.json` 里给一条 `cli` |
...
很多 AI 助手自带命令行，那就不必再复制粘贴。在用户目录
（`%LOCALAPPDATA%\\MinutesDesk`）建一个 `assistants.json`：

```json
[{"name": "Foo",
  "path": "%LOCALAPPDATA%/Foo/Foo.exe",
  "cli":  ["%LOCALAPPDATA%/Foo/Foo.exe", "--cli"]}]
```

`path` 存在时，引擎卡上会直接叫它的名字；再给了 `cli`，那条命令就成为自动路线：
提示词走 stdin（不走命令行参数 —— 一份逐字稿远超 Windows 命令行长度，而且参数对
任何能列进程的东西都可见），正文走 stdout。删掉 `cli` 就回到复制粘贴。
...
草稿不会一遍就对。在纪要那张白纸上划选一句话，会跳出「这里有问题」，写一行为什么
（也可以不写），那句话就带上黄色下划线和编号。纸的上方会数出「N 处待修」，旁边就是
**按标注重写**：助手重读一遍逐字稿，只改你标到的地方，其余段落原样抄回来。它交回来的只是要改的那几句，不是整份文件重打一遍，所以一轮通常十几秒。同一个错在别处也出现的话（决定里人名错了，摘要里通常也错），会一起改掉。如果这场会有中英两份稿子，改动会带到另一份上，不用你在另一页再找一遍同一处。

它不会重跑语音识别 —— 已经确认过的说话人、时间戳、词表修正一个都不动，所以这一步是
几十秒的事，不是几分钟。上一版存成 `minutes.md.bak`，改坏了拿得回来。标注存在纪要
旁边的 `review.json` 里，重排之后还能找回自己的位置，所以可以一次标五处、一轮改完。
点一下黄色下划线就是撤掉那处标注。

引擎没有命令行时，同一个按钮变成**复制修订请求**：还是原来的复制粘贴，只是提示词里
已经带上了这几处标注和你写的原因。

第一次让助手自动写正文（不管是草稿还是重写），工具会明确问你一次：会议内容要交给
`assistants.json` 里那个程序背后的模型，而那个模型不一定在这台机器上。同意过一次，
以后不再问。在 `assistants.json` 里写下一个程序的名字，不等于同意把会议内容交给它 ——
这两件事是分开的。
...
改一份，另一份跟着改。不管是按标注重写、自己在「编辑正文」里改，还是粘回一整份，
改动都会带到另一份稿子上。改的是一个词（术语、人名、系统名），两边是同一个
字符串，直接替换，不花时间。改的是整句话，两种语言之间没法照着替换，所以保存
后会自动多花一次模型调用，大约十几秒，带日志。一次改了十几处以上算重写，
不自动同步，日志里会说。每一次带过去的写入同样留 `.bak`。
